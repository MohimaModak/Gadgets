...
export default function Product() {
    ...
    return (
        <div data-aos="fade-up" id='/Product' className='py-14 px-6 lg:px-20 relative bg-orange-50'>
            ...
            <div className='pt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:gap-6'>
                {Products.map((item, index) => (
                    <div key={index} className='relative'>
                        ...
                        <p className='text-center mt-2 font-semibold'>Price: {item.Price}</p>
                        <div className='flex justify-center items-center'>
                            <button className='bg-sky-600 text-white uppercase px-4 py-2.5 font-semibold mt-2 rounded-md'>Order Now</button>
                        </div>
                    </div>
                ))}
            </div>
